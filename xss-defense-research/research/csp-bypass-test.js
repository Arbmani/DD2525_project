...
async function testCSPWithViolations() {
  ...
  try {
    ...
    // 8. Data URI Bypass Test
    console.log('\n8. Testing if an attacker can use a data URI inside an iframe to execute arbitrary JavaScript');
    await page.evaluate(() => {
      try {
        const iframe = document.createElement('iframe');
        iframe.srcdoc = `<script src="data:text/javascript,alert(1)"></script>`;
        document.body.appendChild(iframe); // Important: actually add the iframe to trigger the violation
        console.log("WARNING: iframe.srcdoc with data URI script was not blocked!");
      } catch (e) {
        console.log("Protected: iframe.srcdoc assignment was blocked:", e.message);
      }
    });

    // 9. JSONP Endpoints Test
    console.log('\n9. Testing if external JSONP endpoints can execute arbitrary JavaScript');
    await page.evaluate(() => {
      try {
    ...
    // 10. Blob URL Bypass Test
    console.log('\n10. Testing if Blob URLs can bypass CSP restrictions');
    await page.evaluate(() => {
      try {
    ...
    // 11. Unsafe Sanitizer Test
    console.log('\n11. Testing if weak Trusted Types policies can be exploited');
    await page.evaluate(() => {
      if (!window.trustedTypes) {
